...
func (p *ProductsDB) handleUpdates() {
	...
	if err != nil {
		p.log.Println("Unable to subscribe for rates", "error", err)
	}
	...
	for {
		rr, err := sub.Recv()
		p.log.Println("Recieved updated rate from server", "dest", rr.GetDestination().String())

		if err != nil {
			p.log.Println("Error receiving message", "error", err)
			return
		}
