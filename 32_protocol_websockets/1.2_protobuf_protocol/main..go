...
var file_person_proto_rawDesc = []byte{
	...
	0x69, 0x61, 0x6c, 0x46, 0x6f, 0x6c, 0x6c, 0x6f, 0x77, 0x65, 0x72, 0x73, 0x42, 0x04, 0x5a, 0x02,
...
func main() {
	...
	data, err := proto.Marshal(ell)
	if err != nil {
		Logg.Println("data err:", data, err)
	}
	Logg.Println(data)
	...
	//// 把数据封装到  空结构体
	err = proto.Unmarshal(data, newEill)

	if err != nil {
		Logg.Fatal("unmarshlling error:", err)

	}

	Logg.Println(newEill.GetAge())

	Logg.Println(newEill.GetName())
	Logg.Println(newEill.GetSocialFollowers())
	Logg.Println(newEill.GetSocialFollowers().GetTwitter())
	Logg.Println(newEill.GetSocialFollowers().GetYoutube())
